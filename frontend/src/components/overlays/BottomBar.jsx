...
export default function BottomBar() {
  const { liveIncidents, setReportPanelOpen, setSosActive, sosActive, isAuthenticated, shareActive } = useAppStore();
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-3 bg-surface/95 backdrop-blur-2xl border border-white/10 rounded-2xl px-4 py-3 shadow-card">
          ...
          {isAuthenticated && (
            ...
            >
              ...
              {shareActive && <span className="text-xs font-semibold">Live</span>}
            </motion.button>
          )}
      ...
      <LocationSharePanel open={sharePanelOpen} onClose={() => setSharePanelOpen(false)} />
    </>
  );
}
